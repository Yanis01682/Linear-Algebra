...
\end{proof}
\begin{rmk}
\replaced{尝试用定义解决第1问，并用矩阵的语言解决第2问。}{分别尝试用矩阵的语言（定义）解决第1（2）问。}
\end{rmk}
...
\end{ex}
\begin{proof}
设向量组$\{a_1, \cdots, a_m\}$的秩为$r$，由定义，向量组中存在一个极大线性无关组，记为$S$，包含向量的个数为$r$。再由极大线性无关组的定义可知，这$r$个向量线性无关。若向量组中还存在多于$r$个线性无关的向量所组成的子向量组，记为$T$，\replaced{则由$S$是极大无关组可知，向量组$\{a_1, \cdots, a_m\}$可由$S$表示，进而}{则由极大无关组的定义可知，}$T$可以被$S$线性表示。结合习题8，有:\replaced{$
r < \mathrm{rank}(T) \leq \mathrm{rank}(S) = r,$}{$r < \mathrm{rank}(S) \leq \mathrm{rank}(T) = r,$}矛盾。
\
...
\end{ex}
\begin{proof}
设$S = \{a_1, \cdots, a_n\}, T = \{b_1, \cdots, b_m\}$。令$S_1$（resp. $T_1$）为$S$（resp. $T$）的任意极大线性无关组，则$\mathrm{rank}(S) = \mathrm{rank}(S_1), \mathrm{rank}(T) = \mathrm{rank}(T_1)$，且由条件，$S_1$ 可以被向量组$T_1$线性表示。由课本命题2.2.10可知，\replaced{$\mathrm{rank}(S_1) \leq \mathrm{rank}(T_1)$}{$\mathrm{rank}(S_1) \leq \mathrm{rank}(S_1)$}。因此$\mathrm{rank}(S) \leq \mathrm{rank}(T)$。
\end{proof}
...
\]
由于$c_1, \cdots, c_r$是$M \cap N$的一组基，故$-(n_1 b_1 + \cdots + n_tb_t0)$可写为$c_1, \cdots, c_r$的线性组合，但在$N$中，\replaced{向量组$\{c_1, \cdots, c_r, b_1, \cdots, b_t\}$}{$c_1, \cdots, c_r$与$b_1, \cdots, b_t$}线性无关，因此
\[
n_1 = n_2 = \cdots = n_t = 0.
\]
代入（\ref{eq10.1}），可知：
