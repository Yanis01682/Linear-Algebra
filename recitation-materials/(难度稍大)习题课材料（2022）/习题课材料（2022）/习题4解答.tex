...
{\em 我们也可利用维数公式说明这一方向的推理}：注意到$\forall x\in \mathbb{R}^n,$ $x = A x + (I_n - A) x.$ 于是$\mathcal{R}(A) + \mathcal{R}(\mathrm{I}_n - A) = \mathbb{R}^n$. 由维数公式知:
	\[
	\mathcal{R}(A) \cap \mathcal{R}(\mathrm{I}_n - A)  = \mathbf{0}.
	\]
而$\mathcal{R }(A (I_n - A)) = \mathcal{R}((I_n - A) A ) \subset \mathcal{R}(A) \cap \mathcal{R}(I_n- A) = \mathbf{0}.$ 于是$A(I_n - A) = 0.$
\end{proof}
\added{
	\begin{proof}
		第二种证明：
		...
		\begin{bmatrix}
			0&I_n\\A^2 - A&0
		\end{bmatrix},$$
因此，$\mathrm{rank}(A)+\mathrm{rank}(I_n-A)=\mathrm{rank}(I_n) + \mathrm{rank}(A^2-A) = n+\mathrm{rank}(A^2-A)$. 于是$\mathrm{rank}(A)+\mathrm{rank}(I_n-A)=n$当且仅当$A^2-A=0$。
	\end{proof}
}
%\begin{ex}
...
\added{
	...
	\end{bmatrix} y= \begin{bmatrix}
		0 \\
		1 
	\end{bmatrix}$无解，即对任意$y\in\mathbb{R}^n$，若$A^Ty=0$，则$b^Ty=0$，换句话说$b\in \mathcal{N}(A^T)^{\perp}=\mathcal{R}(A)$。
\end{proof}
}
\begin{ex}[$\heartsuit $]
	证明多项式
