...
\end{bmatrix}.
\]
由于$A, B$可逆，故$\{a_{i}, 1 \leq i \leq 4\}$和$\{b_{i}, 1 \leq i \leq 4\}$都组成$\mathbb{R}^{2 \times 2}$的一组基。
\item
...
\added{
\begin{proof}[另证]
	可以证明对任意多项式$f(x)$,有$f(A)X=Xf(B)$，取$f$为$A$的特征多项式，则由Hamilton-Cayley定理有$f(A)=0$，又由$f$的根为$A$的特征值，且$A, B$没有公共特征值，有$f(B)$可逆，于是可得$X=0$。
\end{proof}
}
\item
