...
\begin{exercise} \hard
设$A, B$分别是$m\times n, n\times m$矩阵, 证明\added{对任意复数$\lambda$, 有}
\[
...
\end{exercise}
\begin{proof}[证一]
	先取一个$A$的特征向量, 设$A\bs x=\lambda\bs x$. 那么$AB\bs x=BA\bs x=\lambda B\bs x$. 可见$B\bs x$也是$A$的属于相同特征\replaced{值}{子}的特征向量. 
...
\begin{proof}[证二]
	...
	任取$A$的特征对$(\lambda_0,\bs x)$, 则$p_B(B)\bs x=0$. 
	\highlight[id=err,comment={这个貌似不对？如取$A=B=\mu I_n$, 则$p_B(\lambda)=(\lambda-\mu)^n$,$\lambda_1=\cdots=\lambda_n=\mu$. 此时对任意$x$和$k$, 都有$(B-\lambda_{k+1}I)\dots(B-\lambda_nI)\bs x=0$.}]{一定存在$k$}, 使得$(B-\lambda_{k+1}I)\dots(B-\lambda_nI)\bs x\ne0,(B-\lambda_kI)(B-\lambda_{k+1}I)\dots(B-\lambda_nI)\bs x=0$. 
...
设$A$是四阶数独矩阵, 证明其\highlight[comment={这里的绝对值想说的是复数的模长吧？}]{绝对值}最大的特征值为$10$, 且属于该特征值的特征向量的所有分量都相等. 
\end{exercise}
\begin{proof}
    ...
    \textcolor{blue}{要证属于特征值$10$的特征向量各分量皆相等, 仅需讨论上文中不等式的取等号条件即可. 注意到上文中不等式的第一个$\leq$取等号的条件是$x_1,x_2,x_3,x_4$幅角相同, 而第二个$\leq$的取等号条件是$\mid x_1\mid=\mid x_2\mid=\mid x_3\mid=\mid x_4\mid$. 这两个条件放在一起就是$x_1=x_2=x_3=x_4$.}
\end{proof}
...
\begin{proof}[证一]
	...
	\end{bmatrix} \right)$的维数是$r$, 特征子空间$\nullspace(-I-A)=\nullspace\left( \begin{bmatrix}
			-2I_r & \zero \\ -B & \zero
	\end{bmatrix} \right)$的维数是\replaced{$n-r$}{$r$}, 得证. 
\end{proof}
\begin{proof}[证二]
...
\begin{proof}
	\begin{enumerate}
		\item 设$A$是$n$阶方阵. 任选特征对$(\lambda,\bs x)$, 则$\bs 0=(A^2 -A)\bs x=(\lambda^2-\lambda)\bs x$, 可得$\lambda^2-\lambda=0$, 即特征值只能是$0$或$1$. 
			...
			由于$A(I-A)=\zero$, \highlight[comment={Why???}]{故}$\rank(\zero)\ge \rank(A)+\rank(I-A)-n$; 
